...
methods::setMethod("marginalTransitions", signature(model = "rateModel"), function(model,aggregate=c("edge","node"),
  ...
  if(aggregate=="edge"){
    et=model@phylogeny$edgewiseMarginalTransitions(model@alleleData$alleleData@data@x,
                                           model@rateDM@x,
                                           model@piDM@x,threads)
  } else if(aggregate=="node"){
    et=model@phylogeny$nodewiseMarginalTransitions(model@alleleData$alleleData@data@x,
                                                   model@rateDM@x,
                                                   model@piDM@x,threads)
  } else {
