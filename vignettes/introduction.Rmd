...
```{r}
rateMod=rateModel(data = ad,rateFormula=rateFormula,rateBounds = c(10^-5,100))
```
...
plotParams(seTab)
```
If we want to see the distribution of rates for specific covariate values we can compute the rates at desired sites for all edgeGroups and plot them.
```{r}
...
ggplot(rMerged,aes(x=cre.class,y=rate,fill=factor(edgeGroup)))+
  geom_boxplot(notch = TRUE)+
  scale_y_log10(labels = trans_format(log10,math_format()))
```
